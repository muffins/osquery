...
if ($envArch -eq 1) {
  ...
  # Disable compiler errors on warning for 32 bit builds
  $disableErrorOnWarn = "`n`nforeach(flag_var`nCMAKE_CXX_FLAGS CMAKE_CXX_FLAGS_DEBUG " +
                        "CMAKE_CXX_FLAGS_RELEASE CMAKE_C_FLAGS CMAKE_C_FLAGS_DEBUG " +
                        "CMAKE_C_FLAGS_RELEASE CMAKE_CXX_FLAGS_MINSIZEREL " +
                        "CMAKE_CXX_FLAGS_RELWITHDEBINFO CMAKE_C_FLAGS_MINSIZEREL " +
                        "CMAKE_C_FLAGS_RELWITHDEBINFO)`nif(`${flag_var} MATCHES `"/WX`")`n" +
                        "string(REGEX REPLACE `"/WX`" `"`" `${flag_var} `"`${`${flag_var}}`")`n" +
                        "endif(`${flag_var} MATCHES `"/WX`")`nendforeach(flag_var)`n`n"
  Add-Content `
    -NoNewline `
    -Path $cmakePath `
    -Value $disableErrorOnWarn
